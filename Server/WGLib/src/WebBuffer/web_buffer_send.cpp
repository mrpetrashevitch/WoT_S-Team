...
namespace web
{
	namespace io_base
	{
		...
		{
			_need = sizeof(packet::packet_header) + packet->get_header().size;
			_wsa = { (unsigned long)(_need),(CHAR*)&_packet->get_header() };
		}
		...
		packet::packet_network* web_buffer_send::get_packet()
		{
			if (!_front)
				throw;
			return reinterpret_cast<packet::packet_network*>(_front->_packet);
		}
	}
}